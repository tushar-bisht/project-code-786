    ...
    stage("Deploy node app") {
      steps{
        withCredentials([sshUserPrivateKey(credentialsId: 'node-app', keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'ubuntu')]) {
          ...
          sh 'ssh ubuntu@192.168.2.213 -oStrictHostKeyChecking=no docker run -itd -p8080:8080 --name nodeapp 776487083560.dkr.ecr.us-east-1.amazonaws.com/project-code-786:${{BUILD_NUMBER}-1}'
}
}
}
